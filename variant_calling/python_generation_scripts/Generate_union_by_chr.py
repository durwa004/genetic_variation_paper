...
if __name__ == '__main__':
    ...
    tmp = []
    for file_name in os.listdir(data + f"/joint_bcftools_without_Prze/"):
    ...
    for i in tmp:
        ...
        with open(pbs, "w") as f:
            ...
            print(f"java -Xmx4g -jar {gatk}" 
                  + " -T CombineVariants -R "
                  + f"{ref} "
                  + f"--variant:bcftools joint_bcftools_without_Prze/" + i + ".genotyped.vcf.gz --variant:gatk joint_gatk_without_Prze/" + i + ".genotyped.vcf.gz -o joint_union/" + i + "_union_joint.vcf.gz -nt 4 -genotypeMergeOptions UNSORTED", file=f, sep="") 
