...
if __name__ == '__main__':
    ...
    info = data + "/breed_rare_common_number_of_variants.txt"

    with open(info, "w") as info_file, open("bcftools_query_chrom_pos.sh", "w") as output_f:
        ...
        for entry in os.listdir(data):
            if os.path.isdir(data + "/" + entry) == True:
                for file_name in os.listdir(data + "/" +entry):
                    if "0002.vcf.gz.stats" in file_name:
                        ...
                        print(f"bcftools query -f '%CHROM\ xxxxxt%POS\ xxxxxn' {file_name} > {data}/breed_rare_common_chrom_pos/{entry}.txt", file = output_f)
